...
// Si el error proviene de Joi significa que la informacion enviada en la peticion es incorrecta o incompleta
  ...
  } else if (err instanceof createError.HttpError) { // Si el error es de tipo httpError, es un tipo de error esperado y el mensaje puede ser mostrado a los usuarios
  ...
  } else { // Si el error llega a esta instancia, es un error inesperado en el servidor y no debe ser revelado al usuario, se le envia el codigo y un mensaje para remitirlo a nosotros
    res.status(err.status || 500).json({ 
      error: {
        codigo: randomErrorId,
        message:
